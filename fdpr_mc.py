...
fdpr_mc.py
==========
Monte Carlo sweep using the fixed pipeline from fdpr_injectable.py.

Grid axes:
  Axis 0 — dz  : single defocus distance [mm] per trial
                  (tests how reconstruction quality depends on defocus amount)
  Axis 1 — param: aberration parameter
                  sinusoid → v0 [cycles/aperture]
                  zernike  → amplitude [waves]

At each (dz, param) grid point, N_TRIALS noise realisations are run and
RMS statistics are stored.  Results are saved as .npz and displayed as
heatmaps, matching the output format of fdpr_vs_AS_save.py.
...
# --- Aberration type ---
ABERRATION_TYPE = "sinusoid"   # "sinusoid"  or  "zernike"

# Zernike: fixed mode index, amplitude swept on Axis 1
ZERNIKE_MODE_INDEX = 6         # 6 = coma

# --- Grid ---
# Axis 0: defocus distances [mm] — one defocus image per trial
DZ_GRID_MM = np.linspace(5, 100, 10)

# Axis 1: aberration parameter
#   sinusoid → spatial frequency [cycles/aperture]
#   zernike  → amplitude [waves]
SINUSOID_V0_GRID   = np.linspace(0.5, 20, 10)   # cycles/aperture
SINUSOID_AMP_WV    = 0.5                          # fixed amplitude for sinusoid sweep
ZERNIKE_AMP_GRID   = np.linspace(0.05, 1.0, 10)  # waves
...
# --- Output ---
SAVE_LABEL = f"fdpr_mc_{ABERRATION_TYPE}.npz"
...
def run_fdpr(psf_focused, psf_defoc, dz_mm):
    """
    Run FocusDiversePhaseRetrieval for a single focused + defocused PSF pair.
    ...
    psfd = add_read_noise(psf_defoc)   if IF_NOISE else psf_defoc.copy()

    if not (np.all(np.isfinite(psf0)) and np.all(np.isfinite(psfd))):
        return None
...
if ABERRATION_TYPE == "sinusoid":
    param_grid  = SINUSOID_V0_GRID
    param_label = "v₀ [cycles/aperture]"
    param_name  = "v0"
elif ABERRATION_TYPE == "zernike":
    param_grid  = ZERNIKE_AMP_GRID
    param_label = f"Amplitude [waves]  (Z mode {ZERNIKE_MODE_INDEX})"
...
print(f"\n{'='*55}")
print(f"FDPR Monte Carlo  —  {ABERRATION_TYPE}")
print(f"  Grid  : {Ndz} dz × {Np} {param_name} = {Ndz*Np} points")
print(f"  Trials: {N_TRIALS}   Iterations: {N_ITER}   Noise: {IF_NOISE}")
if ABERRATION_TYPE == "sinusoid":
    print(f"  Fixed amplitude: {SINUSOID_AMP_WV} waves")
print(f"  dz range    : {DZ_GRID_MM[0]:.1f} – {DZ_GRID_MM[-1]:.1f} mm")
print(f"  param range : {param_grid[0]:.2f} – {param_grid[-1]:.2f}")
print(f"{'='*55}\n")
...
for i, dz_mm in enumerate(DZ_GRID_MM):
    ...
    for j, param in enumerate(param_grid):
        ...
        if ABERRATION_TYPE == "sinusoid":
            phi_ab = make_sinusoidal_phase(float(param), SINUSOID_AMP_WV)
        else:
            phi_ab = make_zernike_phase(ZERNIKE_MODE_INDEX, float(param))
        ...
        psf_defoc   = psf_from_wavefront(make_wavefront(phi_ab + phi_def))

        if not (np.all(np.isfinite(psf_focused)) and np.all(np.isfinite(psf_defoc))):
            print(f"  Non-finite PSF at dz={dz_mm:.1f}, param={param:.2f} — skipping")
            continue

        trial_rms = []
        for t in range(N_TRIALS):
            recon = run_fdpr(psf_focused, psf_defoc, dz_mm)
...
np.savez(
    ...
    zernike_mode = ZERNIKE_MODE_INDEX,
    sinusoid_amp = SINUSOID_AMP_WV,
)
...
heatmap(rms_mean,  "Mean Residual RMS  (truth − recon)", "RMS [nm]")
...
ax.set_title(f"RMS vs {param_name}  at selected dz values")
ax.legend()
