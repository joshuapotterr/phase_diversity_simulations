...
'''
DEFOCUS_MODEL = "pupil_zernike"   # you can choose: "pupil_zernike" OR "image_AS"
PUPIL_READOUT = "mft_rev"         # you can choose: "mft_rev" OR "hcipy_backward"
try:
...
def plot_pupil_convergence(snapshot_data, conf, telescope_pupil):
    ...
    plt.subplot(1, 2, 1)
    plt.imshow(snapshot_data["psf_rec_complex"].real)
    plt.subplot(1,2, 2)
    plt.imshow(snapshot_data["psf_rec_complex"].imag)
    plt.show()
...
def plot_psf_convergence(snapshot_data):
    ...
    for ax, it in zip(axes, iters):
        ...
        ax.imshow(np.log10(psf + 1e-10), cmap="inferno", vmin=-6)
        ax.set_title(f"Iter {it}")
        ax.axis("off")
        ...
        for t in range(N_trials):
            ...
            try:
                ...
                if snapshot:
                    ...
                    '''
                    plts the amplitude and phase information of psf_reconstruction'''
                    plot_pupil_convergence(snapshot_data, conf, telescope_pupil)
