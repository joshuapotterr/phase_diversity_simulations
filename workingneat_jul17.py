...
def convert_psf_estimate_to_phase(psf_estimate,
    ...
    seal_configuration = InstrumentConfiguration(seal_params)
...
def check_phase_estimate(system_truth_phase, phase_estimate,masking_pupil):
    ...
    #system_truth_phase = resize(system_truth_phase, (256,256))
    ...
    ##check if med subtracted is nan
    plt.figure()
...
def make_cost_functions_plots(cost_functions, filename='Jul_9.jpg'):
    """
...
def run_phase_retrieval(system_truth_intensity,
    ...
    try: 
        ...
        distance_list_um = [d *1e3 for d in distance_list]# mm to micron
        #could use sorted here instead, psfs and sitances may mismatch unless order
        #is guaranteed by construction
        print("defocus distance_list:", distance_list)
        ...
        print('shape of system_truth_intensity', system_truth_intensity.shape)
        psf_list = [system_truth_intensity] +[defocus_dictionary[key] for key in distance_list]

        for i, (d, psf) in enumerate(zip([0.0] + distance_list, psf_list)):
            print(f"PSF[{i}] @ Δz={d:.4e} m  →  "
                f"min={psf.min():.3e}, max={psf.max():.3e}, sum={psf.sum():.3e}, "
                f"nonzero={np.count_nonzero(psf)}/{psf.size}")

        # 1. Basic textual summary
        print(f"Total PSFs: {len(psf_list)}")
        for i, psf in enumerate(psf_list):
            print(f" PSF[{i:2d}]: shape={psf.shape}, dtype={psf.dtype}, "
                f"min={psf.min():.3e}, max={psf.max():.3e}, sum={psf.sum():.3e}, "
                f"nonzero={np.count_nonzero(psf)}/{psf.size}")
        # If you also have a distance_list of defocus offsets, zip them:
        if 'distance_list' in globals():
            for d, psf in zip(distance_list, psf_list):
                print(f"  distance list = {d:.3e} m → PSF max {psf.max():.3e}")
        for psf in psf_list:
            print(psf.shape)
    #    for psf in psf_list:
    #       imshow_field(psf)
    ...
    #    print('type of psf_list', type(psf_list))
        print("defocus_dictionary keys in run phase retireval:", list(defocus_dictionary.keys()))
        ...
        for key, value in zip(distance_list, psf_list):
            plt.imshow(value)
            plt.title(f'{key} PSF in psf_list')
            plt.show()

        mp= FocusDiversePhaseRetrieval(psflist= psf_list,
                                    wvl= seal_parameters['wavelength_micron'], #JARENS NOT IN METER
                                    dxs =dx_list, 
                                    defocus_positions= distance_list_um)
...
def calculate_phase_retrieval_accuracy(
    ...
    plt.imshow(phase_estimate, cmap='RdBu')
    plt.colorbar(label='Phase [rad]')
    plt.title('phase_estimate, after call to convert_psf')
    plt.show()
...
def simulate_focused_image(wf_error_to_retrieve, 
    ...
    wf_focused_intensity = wf_focused.intensity.shaped
    wf_pupil_phase = wf_focused_pupil.phase
    plt.imshow(wf_focused_intensity)
    plt.title('system_truth_inten in focused image')
    plt.show()
    plt.imshow(wf_pupil_phase.shaped)
    plt.title('wf_pupil_phase in focused image')
    plt.show()
...
def simulate_defocused_image(defocus_phase,
    ...
    print('defocus phase during simulate_defocused_image is', defocus_phase)
    plt.figure()
    plt.imshow(defocus_phase)
    plt.title('defocus phase before added to aberration to impart')
    plt.show()
    aberration_to_impart = wf_error_to_retrieve + defocus_phase


    wavefront_defocused = Wavefront(simulation_elements['telescope_pupil'] * np.exp(1j * aberration_to_impart.ravel()),#or flatten
                           seal_parameters['wavelength_meter'])
    prop2f = FraunhoferPropagator(simulation_elements['pupil_grid'],
                                    simulation_elements['focal_grid'],
                                    focal_length=seal_parameters['focal_length_meters']
                                    )
    aberrated_psf = prop2f(wavefront_defocused)
    plt.figure
    plt.imshow(aberrated_psf.intensity.shaped)
    plt.title('aberrated psf intensity')
    plt.colorbar()
    plt.show()
...
def calculate_defocus_phase(seal_parameters,
    ...
    defocus_distance_m = defocus_distance*1e-3# mm to meter
    print(f'defocus distance is {defocus_distance}')

    defocus_p2v = delta_to_p(
                            delta = defocus_distance_m,
                            f = seal_parameters['focal_length_meters'],
                            D=seal_parameters['pupil_size']
                            )
...
def focus_diverse_phase_retrieval(system_truth_intensity,
    ...
    for defocus_distance in defocus_distances:
        ...
        print("defocus phase P2V =",
                defocus_phase.max() - defocus_phase.min(),
                "rad")
        ...
        im = plt.imshow(defocused_psf,
                        #norm=LogNorm(vmin=defocused_psf.max()*1e-6,
                        #            vmax=defocused_psf.max()),
                        origin='lower')
        plt.title(f"Defocused PSF @ distance = {defocus_distance:.2f} µm")
        plt.axis('off')
        plt.colorbar()
        plt.show()
...
def simulate_phase_diversity_grid(wf_error_to_retrieve,
    ...
    for simulation_specifics in phase_diverse_inputs:
        ...
        print("defocus_dictionary keys in phase diverse inputs loop:", list(defocus_dictionary.keys()))
        #indices first half
        ...
        #took out creation of empty defocus_dictionary here
        for defocus_distance in defocus_distances:
        ...
        print("defocus dict KEYS in sim grid:", defocus_dictionary.keys() if hasattr(defocus_dictionary, 'keys') else 'not a dict')
        ...
        psf_estimate, cost_functions = focus_diverse_phase_retrieval(system_truth_intensity, 
                                                                     ...
                                                                     seal_parameters, 
                                                                     simulation_elements)
        print('PSF ESTIMATE:',psf_estimate)
...
def main(seal_parameters,
    ...
    plt.imshow(wf_error_to_retrieve)
    plt.title('wf error to retrieve MAIN')
    plt.show()
...
if __name__ == "__main__": 
    ...
    seal_params = {'image_dx': 2.0071, # 
               'efl': 500e-3*1e3, # SEAL effective focal length, mm
               'wavelength': 0.65, # SEAL center wavelength, microns
                'pupil_size': 10.12e-3*1e3, # Keck entrance pupil diameter
                    }
    ...
    upper_triangle = (np.array([5]), np.array([8]))
    ...
    for index in upper_triangle_list:
        ...
        #this needs the new defocus distances 
        print("dict keys:", sorted(defocus_dictionary.keys()))
        for dz, arr in defocus_dictionary.items():
            print(f"{dz:.3e}: min={arr.min():.3e}, max={arr.max():.3e}")

        print('defocus dict during indexing triangle',defocus_dictionary)
