...
def figure_dean_bowers_theory(data: dict, v0_select: float = 15.0,
                               save_path: Optional[str] = None):
    """
    Compare measured OTF amplitude to Dean & Bowers sin² theory.
    
    Uses higher v0 (default 30) where sampling is adequate to resolve
    the oscillations. At low v0, theory oscillates too rapidly for 80-point sampling.
    """
    dz = data['dz']
    v0 = data['v0']
    H = data['H_photometry']
    ...
    # High-resolution theory curve for smooth plotting
    a_hat_fine = np.linspace(a_hat.min(), a_hat.max(), 500)
    dz_fine = a_hat_to_dz(a_hat_fine)
    arg_fine = np.pi * actual_v0**2 / (8 * np.maximum(a_hat_fine, 0.01))
    theory_fine = np.abs(np.sin(arg_fine))
    
    # Theory at measured points for correlation
    arg = np.pi * actual_v0**2 / (8 * np.maximum(a_hat, 0.01))
    theory_at_measured = np.abs(np.sin(arg))
    ...
    # Compute correlation
    valid_both = valid & np.isfinite(theory_at_measured)
    corr = np.corrcoef(measured_norm[valid_both], theory_at_measured[valid_both])[0, 1]
    
    # Left: Plot vs â
    ax = axes[0]
    
    ax.plot(a_hat_fine, theory_fine, 'r-', lw=2, 
            label=r'Theory: $|\sin(\pi v_0^2 / 8\hat{a})|$', zorder=2)
    ax.plot(a_hat[valid], measured_norm[valid], 'o-', color=COLORS['blue'],
            lw=1.5, ms=5, alpha=0.8, label='Measured (normalized)', zorder=3)
    ...
    ax.set_xlim(10, a_hat.max())  # Focus on well-sampled region
    ...
    ax.plot(dz_fine, theory_fine, 'r-', lw=2, label='Theory', zorder=2)
    ax.plot(dz[valid], measured_norm[valid], 'o-', color=COLORS['blue'],
            lw=1.5, ms=5, alpha=0.8, label='Measured', zorder=3)
    
    # Mark theoretical maxima (only within range and limit count)
    n_shown = 0
    for n in range(1, 20):
        a_max = actual_v0**2 / (4 * (2*n - 1))
        dz_max = a_hat_to_dz(a_max)
        if dz.min() <= dz_max <= dz.max() and n_shown < 5:
            ax.axvline(dz_max, color='gray', linestyle=':', alpha=0.5)
            ax.text(dz_max, 1.08, f'n={n}', ha='center', fontsize=9, color='gray')
            n_shown += 1
...
def generate_all_paper_figures(npz_path: str, output_dir: str = "./FDPRNotebooks",
    ...
    print("\n[5/7] Dean & Bowers theory comparison (v0=30 for good sampling)...")
    figure_dean_bowers_theory(data, v0_select=30, save_path=f"{output_dir}/fig5_dean_bowers.{fmt}")
