...
CONFIGURATION FLAGS
'''
verbose = True          # If True, plot everything step by step
spot_check = True        # If True, do detailed plot at specific (dz, v0)
...
'''
dzs_mc = np.linspace(5, 250, 4)      # mm
v0s_mc = np.linspace(0.5, 20, 4)     # cycles/aperture
...
def plot_spot_check(dz_mm, v0, phi_sine_rad, phi_def, psf_focus_clean, psf_defoc_clean,
                    psf0, psfd, psf00, real_pupil, truth_phase, residual,
    ...
    ax = axes[2, 0]
    im = ax.imshow(np.log10(np.abs(psf00)**2 + 1e-10), vmin=-5, cmap='inferno')
...
for i, dz in enumerate(dzs_mc):
    ...
    for j, v0 in enumerate(v0s_mc):
        ...
        if verbose and i % 5 == 0 and j % 10 == 0:  # Plot every 5th dz and 10th v0
        ...
        for t in range(N_trials):
            ...
            try:
                ...
                truth_phase = phi_sine_rad.shaped
