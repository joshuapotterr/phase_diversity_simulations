...
def resize_psf(psf, target_shape=(256, 256)):
    return resize(psf, target_shape, mode='reflect', anti_aliasing=True)
...
def check_phase_estimate(system_truth_phase, phase_estimate,masking_pupil):
    ...
    true_phase = resize(true_phase, (256,256))
    ...
    #plt.imshow(difference_masked)
    plt.figure(figsize=(6, 5))
    plt.imshow(difference_true_vs_estimate, origin='lower')
    plt.colorbar(label='Phase Difference [rad]')
    plt.title('difference_true_vs_estimate')
    plt.xlabel('X')
    plt.ylabel('Y')
    plt.tight_layout()
    plt.show
...
def run_phase_retrieval(system_truth_intensity,
    ...
    plt.figure()
    plt.imshow(np.angle(psf_estimate), cmap='viridis')
...
def calculate_phase_retrieval_accuracy(
    ...
    plt.colorbar(label='Phase [rad]')
    plt.title('Retrieved Pupil Phase, after call to convert_psf')
    plt.show()
...
def simulate_focused_image(wf_error_to_retrieve, 
    ...
    wf_clean = Wavefront(telescope_pupil, wavelength)
    pupil_image=wf_clean.copy()
    pupil_image.electric_field = np.exp(complex(0,1)*(wf_error_to_retrieve.ravel()))*telescope_pupil

    wf_focused = prop2f(pupil_image)
    wf_focused_intensity = wf_focused.intensity
    wf_focused_phase = wf_focused.phase
    wf_focused_focal = prop2f(pupil_image)
    imshow_field(np.log10(wf_focused_focal.intensity / wf_focused_focal.intensity.max()), vmin=-5)
    wf_focused_intensity = resize_psf(wf_focused_intensity, (256,256))
...
def simulate_defocused_image(defocus_phase,
    ...
    pupil_image = Wavefront(simulation_elements['telescope_pupil'], seal_parameters['wavelength_meter'])
    aberration_to_impart = wf_error_to_retrieve + defocus_phase
    pupil_image.electric_field = np.exp(complex(0, 1)*(aberration_to_impart.ravel()))*simulation_elements['telescope_pupil']
    ...
    imshow_field(np.log10(aberrated_psf.intensity / aberrated_psf.intensity.max()), vmin=-5)
