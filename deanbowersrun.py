...
if __name__ == "__main__": 
    ...
    wavelength=seal_parameters['wavelength']
    
    ###DEAN BOWERS TEST
    a_hats = dean_bowers_max_list(fringes=11, max_n=2, wavelength=seal_parameters['wavelength'])
    ...
    fringes = 21
    max_n = 1  # or seal_parameters['grid_dim'] - 1
    dean_deltas = dean_bowers_defocus_deltas(fringes, max_n, f, D, wavelength)
    dim = len(dean_deltas)
    seal_parameters['grid_dim'] = dim

    phase_diverse_inputs = []

    # Upper triangle indices
    upper_triangle = np.triu_indices(dim, k=1)
    upper_triangle_pairs = zip(upper_triangle[0], upper_triangle[1])

    for i, j in upper_triangle_pairs:
        d1, d2 = dean_deltas[i], dean_deltas[j]

        defocus_dict = {
            d1: calculate_defocus_phase(seal_parameters, simulation_elements, d1),
            d2: calculate_defocus_phase(seal_parameters, simulation_elements, d2)
        }
        phase_diverse_inputs.append((i, j, defocus_dict))
main(seal_parameters,
     phase_diverse_inputs,
     file_name_out ='jul16_dean.npy',
     heatmap_plot_out= 'dean_heatmap.png')