...
fdpr_mc_pm.py
=============
Monte Carlo sweep — identical to fdpr_mc.py except that each grid point
uses BOTH +dz and −dz defocused PSFs (±-diversity), which breaks the
sign ambiguity that causes high residuals in the single-defocus version.

PSF list passed to FocusDiversePhaseRetrieval:
  [psf_focused, psf_defoc_pos, psf_defoc_neg]   (length 3)
  dx_list      = [IMAGE_DX_UM, IMAGE_DX_UM]      (length 2)
  defocus_list = [+dz_um, -dz_um]                (length 2)

Everything else — grids, noise model, RMS metric, plots — is unchanged
from fdpr_mc.py.
...
# --- Output ---
SAVE_LABEL = f"fdpr_mc_pm_{ABERRATION_TYPE}.npz"
...
print(f"\n{'='*55}")
print(f"FDPR Monte Carlo (±dz diversity)  —  {ABERRATION_TYPE}")
