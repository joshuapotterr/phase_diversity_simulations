...
from image_sharpening import GerchbergSaxtonFixedSampling,ConjugateGradientFixedSampling,FocusDiversePhaseRetrieval
...
if __name__ == '__main__':
    ...
    defocus_dz = [defocus_coeff_to_distance(P,13.66)*(32/13.66) for P in defocus_coeffs] # 
    ...
    for i,z4 in enumerate(defocus_coeffs):
        # take multiple exposures and combine into single PSF
        
        # poke z4
        DM.pokeZernike(z4,4,bias=dm_flat)
        ...
        if i == 0:
            ...
            centered_data = shift(cropped_data,computed_center)
            
        
        # TODO: EXPERIMENTAL apply Gaussian filter
        if gaussian_filter:
            centered_data = gaussian_filter_psf(centered_data,waist=6)
        ...
        # pr = GerchbergSaxtonFixedSampling(cropped_data,pupil_amplitude)
        
        psf_list.append(centered_data)
        
    
    # re-apply dm map
    DM.setSurf(dm_flat)
    
    pr = FocusDiversePhaseRetrieval(psf_list,wavelength*1e6,image_dx_um[1:],defocus_dz[1:],fno)
    
    for j in range(niters):
        psf_estimation = pr.step()
    
    pupil_estimation = mft_rev(psf_estimation)*pupil_amplitude
    
    #gs = GerchbergSaxtonFixedSampling(psf_list[0],pupil_amplitude,phase_guess=np.angle(pupil_estimation))
    
    #for j in range(niters):
    #    pupil_estimation = gs.step()
        
    psf_estimation = mft_fwd(pupil_estimation)
    
    plt.figure()
    plt.subplot(121)
    plt.imshow(unwrap_phase(np.angle(pupil_estimation)),cmap='RdBu_r')
    plt.colorbar()
    plt.subplot(122)
    plt.imshow(np.log10(np.abs(psf_estimation)**2),cmap='inferno',vmin=3)
    plt.colorbar()
    plt.show()
    # sys.exit('Breakpoint')
    ...
    dm_map = sum_of_2d_modes(dm_norm_basis,coeffs)
    ...
    print((data_maximum - np.max(corrected_psf))/data_maximum)
    ...
    plt.subplot(131)
    plt.title('PSF')
    plt.imshow((cropped_after),cmap='inferno',vmin=-4)
